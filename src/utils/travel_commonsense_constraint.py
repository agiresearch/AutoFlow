from utils.travel_utils import get_valid_name_city,extract_before_parenthesis,extract_numbers_from_filenames
...
def is_reasonalbe_visiting_city(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        if type(tested_data) != list or 'current_city' not in tested_data[i]:
            return False, "No current city exists."
...
def is_valid_transportation(question, tested_data):
    ...
    if 'transportation' in tested_data[0] and tested_data[0]['transportation'] and tested_data[0]['transportation'] != '-':
...
def is_valid_days(question, tested_data):
    ...
    lens = 0
    for i in range(min(question['days'],len(tested_data))):
        if tested_data[i] != {} and 'current_city' in tested_data[i] and \
                tested_data[i]['current_city'] != "You don't need to fill in the information for this or later days.":
            lens += 1
