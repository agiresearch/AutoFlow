...
from utils.travel_commonsense_constraint import evaluation as commonsense_eval
from utils.travel_hard_constraint import evaluation as hard_eval
...
def load_file_json_data(filepath, mode):
    data = []
    if mode == 'train':
        file_cnt = 45
    elif mode == 'test':
        ...
        file_cnt = 180
    for idx in range(1, file_cnt + 1):
        plan = load_line_json_data(os.path.join(filepath, mode, f'plan_{idx}.json'))[0][0]
        if 'plan' in plan:
            data.append({'plan': plan['plan']})
    return data
...
def eval_score(set_type: str, file_path: str, cache_dir: str):

    if set_type == 'train':
        query_data_list = load_dataset('osunlp/TravelPlanner','train',download_mode="force_redownload", cache_dir=cache_dir)['train']
    elif set_type == 'validation':
        query_data_list = load_dataset('osunlp/TravelPlanner','validation',download_mode="force_redownload", cache_dir=cache_dir)['validation']
    else:
    ...
    if file_path.endswith('json') or file_path.endswith('jsonl'):
        ...
        tested_plans = load_file_json_data(file_path, set_type)
    delivery_cnt = 0
