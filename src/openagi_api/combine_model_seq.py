...
class SeqCombine:
    ...
    def construct_module_tree(self, module_list):
        # self.module_function_list = []
        self.module_list = []
        self.auto_used_device_list = dict()
        i = 0
        cur_device = self.device_list[i]

        for module in module_list:
            module = module.strip()
            if not module.startswith('Input'):
                temp_values = self.module2function_dict[module]
                temp_m = temp_values[1]
                if cur_device != "cpu":
                    if torch.cuda.mem_get_info(cur_device)[0] / 1024 ** 3 < 3:
                        ...
                        cur_device = self.device_list[i]
                temp_m = temp_m.to(cur_device)
                self.auto_used_device_list[module] = cur_device
                self.module_list.append(temp_m)

    def run_module_tree(self, module_list, input_data, data_type):
        image_data, text_data = [], []
        for i, dt in enumerate(data_type):
            if 'text' in dt:
                text_data.append(input_data[i])
            elif 'image' in dt:
                image_data.append(input_data[i])
            else:
        ...
        for module in module_list:
            ...
            if module.startswith('Input'):
                ...
                if 'Image' in module:
                    if len(image_data) > 1:
                        index = int(module.split(' ')[-1])
                    variables.append(image_data[index])
                elif 'Text' in module:
                    if len(text_data) > 1:
                        index = int(module.split(' ')[-1])
                    variables.append(text_data[index])
                else:
                    raise NotImplementedError
            else:
                temp_values = self.module2function_dict[module]
                if 'Question Answering' in module:
                    data = [variables.pop()]
                    data.append(variables.pop())
                else:
                    data = variables.pop()
                temp_f = eval(temp_values[0])
                new_data = temp_f(data, self.auto_used_device_list[module])
                variables.append(new_data)
    ...
    def vqa(self, input_data, device):
        ...
        encoding = self.vqa_processor(imgs, questions, return_tensors="pt", padding=True, truncation=True).to(device)
        # forward pass
        with torch.no_grad():
    ...
    def sentiment_analysis(self, sentences, device):
        ...
        """
        inputs = self.sentiment_analysis_tokenizer(sentences, return_tensors="pt", padding=True, truncation=True).to(device)
    ...
    def question_answering(self, input_data, device):
        ...
        inputs = self.question_answering_tokenizer(questions, contexts, return_tensors="pt", padding=True, truncation=True).to(device)
    ...
    def fill_mask(self, sentences, device):
        ...
        inputs = self.unmask_tokenizer(sentences, return_tensors="pt", padding=True, truncation=True).to(device)
        ...
        results = []
        for i in range(batch_size):
            input_list = inputs.input_ids[i].tolist()
            if self.unmask_tokenizer.mask_token_id not in input_list:
                results.append(sentences[i])
            else:
                # Get the top 5 tokens and their probabilities for the masked position
                masked_index = input_list.index(self.unmask_tokenizer.mask_token_id)
